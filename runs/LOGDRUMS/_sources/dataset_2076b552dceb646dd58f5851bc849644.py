...
class PianoRollAudioDataset(Dataset):
    def __init__(self, path, groups=None, sequence_length=None, seed=42, device=DEFAULT_DEVICE):
        ...
        for group in groups:
            for input_files in tqdm(self.files(group), desc='Loading group %s' % group):
                ...
                if length < SEQUENCE_LENGTH and len(self.data) == 0:
                    continue

                if length < SEQUENCE_LENGTH and len(self.data) != 0:
                    continue
                    random = np.random.randint(0, len(self.data))
                    self.data[random]['path'] = self.data[random]['path'] + files['path']
                    self.data[random]['audio'] = torch.cat((self.data[random]['audio'], files['audio']))
                    self.data[random]['label'] = torch.cat((self.data[random]['label'], files['label']))
                    self.data[random]['velocity'] = torch.cat((self.data[random]['velocity'], files['velocity']))
                else:
    ...
    def __getitem__(self, index):
        ...
        if self.sequence_length is not None:
            ...
            # if (step_begin + n_steps) > data['label'].shape[0]:
            #     step_begin = step_begin - ((step_begin + n_steps) - data['label'].shape[0])
...
class GROOVE(PianoRollAudioDataset):
    def __init__(self, path='data/GROOVE_441', groups=None, sequence_length=None, seed=42, device=DEFAULT_DEVICE):
        super().__init__(path, groups if groups is not None else ['test_linear_lower_40db'], sequence_length, seed, device)
